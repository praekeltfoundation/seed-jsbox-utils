...
Check that number (normalized or not) is a phone number that starts with 0/+ has appropriate length
...
is_valid_msisdn = function(number, min_length, max_length) {
    var number_to_check = number[0] === '+'
        ? number.substring(1, number.length)
        : number;

    return check_valid_number(number_to_check)
        && (number[0] === '0' || number[0] === '+')
...
/**: get_today(config)
Returns today's date unless testing date is specified in config, that will be returned.

:param object config: Optional; should contain 'testing_date' property
*/
...
get_january = function(config) {
    ...
    return this.get_today(config).startOf('year');
};
...
*/
is_valid_day_of_month = function(day) {
...
/**: is_weekend(config)
Checks if today is a weekend according to UTC (Coordinated Universal Time).
If config with 'testing_date' parameter is specified, the alternate date will be
checked.

:param object config: Optional.
*/
is_weekend = function(config) {
    var today = this.get_today(config);
...
:param string valueFormat: required value format
:param string labelFormat: require label format
*/
