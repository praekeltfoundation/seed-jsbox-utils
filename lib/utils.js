...
:param string msisdn: msisdn to process
...
:param string format: Optional. Specifies the format of testing date in config
    e.g. 'YYYY-MM-DD hh:mm:ss', "YYMMDD", etc.  Defaults to 'YYYY-MM_DD'
*/
get_today = function(config, format) {
    var date_format = format || 'YYYY-MM-DD';  // if format not specified, assume default

    if (config) {
        return new moment(config.testing_today, date_format);
    } else {
        return new moment();
    }
};
