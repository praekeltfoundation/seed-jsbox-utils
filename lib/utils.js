...
/*check_fixtures_used2 = function(expected_used) {
    var fixts = fixtures();
    var fixts_used = [];
    fixts.forEach(function(f, i) {
        f.uses > 0 ? fixts_used.push(i) : null;
    ...
    switch (method) {
            case "post":
                return http.post(im.config.services[service].url + endpoint, {
                    ...
                    .then(log_service_api_call(service, method, params, payload, endpoint, im));
            case "get":
...
create_registration = function(im, reg_info) {
    return service_api_call("registrations", "post", null, reg_info, "registration/", im)
...
get_identity_by_address = function(address, im) {
  ...
  // {'msisdn': '0821234444'}; {'email': 'me@example.com'}
    ...
    return im
        ...
        .then(function() {
            return service_api_call('identities', 'get', params, null, 'identities/search/', im)
...
get_identity = function(identity_id, im) {
  ...
  // Returns the identity object
    ...
    return service_api_call('identities', 'get', {}, null, endpoint, im)
        .then(function(json_get_response) {
...
create_identity = function(im, address, communicate_through_id, operator_id) {
  ...
  // Returns the identity object
    ...
    return service_api_call("identities", "post", null, payload, 'identities/', im)
...
get_or_create_identity = function(address, im, operator_id) {
  // Gets a identity if it exists, otherwise creates a new one
    ...
    /*if (address.msisdn) {
        address.msisdn = normalize_msisdn(address.msisdn, im.config.country_code);
    }*/

    return get_identity_by_address(address, im) // Get identity id using address
        .then(function(identity) {
            if (identity !== null) {
                ...
                return create_identity(im, address, null, operator_id)
                .then(function(identity) {
                    return identity;
                });
            }
...
update_identity = function(im, identity) {
  ...
  // Returns the id (which should be the same as the identity's id)
    ...
    return service_api_call('identities', 'patch', {}, identity, endpoint, im)
...
get_subscription = function(im, subscription_id) {
  ...
  // Returns the subscription object
    ...
    return service_api_call('subscriptions', 'get', {}, null, endpoint, im)
        .then(function(response) {
...
get_active_subscriptions_by_identity = function(im, identity_id) {
  ...
  // Returns the first subscription object found or null if none are found
    ...
    return service_api_call('subscriptions', 'get', params, null, endpoint, im)
        .then(function(response) {
...
get_active_subscription_by_identity = function(im, identity_id) {
  ...
  // Returns the first subscription object found or null if none are found
    ...
    return get_active_subscriptions_by_identity(im, identity_id)
...
update_subscription = function(im, subscription) {
  ...
  // Returns the id (which should be the same as the subscription's id)
    ...
    return service_api_call('subscriptions', 'patch', {}, subscription, endpoint, im)
        .then(function(response) {
            return response.data.id;
        });
};
...
save_inbound_message = function(im, from_addr, content) {
  // Saves the inbound messages to seed-message-sender
    ...
    return service_api_call("message_sender", "post", null, payload, 'inbound/', im)
        .then(function(json_post_response) {
...
optout = function(im, identity_id, optout_reason, address_type, address,
  ...
  // Posts an optout to the identity store optout endpoint
    ...
    return service_api_call("identities", "post", null, optout_info, "optout/", im)
        .then(function(response) {
            return response;
        });
};
...
module.exports = {
    ...
    log_service_api_call: log_service_api_call,
    ...
    get_identity_by_address: get_identity_by_address,
    get_identity: get_identity,
    create_identity: create_identity,
    get_or_create_identity: get_or_create_identity,
    update_identity: update_identity,
    get_subscription: get_subscription,
    get_active_subscriptions_by_identity: get_active_subscriptions_by_identity,
    get_active_subscription_by_identity: get_active_subscription_by_identity,
