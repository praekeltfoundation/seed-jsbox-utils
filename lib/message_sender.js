...
MessageSender.prototype = {
    ...
    */
    ...
    /**:MessageSender.create_outbound_message(outbound_msg_info)
    Creates the outbound messages to seed-message-sender

    :param object outbound_msg_info:
      Gets passed through directly to service. Consult service docs/protocol.
    ...
    */
    create_outbound_message: function(outbound_msg_info) {
        var endpoint = 'outbound/';
        var url = this.base_url + endpoint;

        return this.http_api.post(url, {data: outbound_msg_info})
            .then(this.log_service_call('POST', url, outbound_msg_info, null))
            .then(function(response) {
                return response.data;
            });
    },
