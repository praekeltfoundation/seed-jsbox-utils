...
Hub.prototype = {
    ...
    */
    create_registration: function(reg_info) {
        ...
        return this.http_api.post(url, reg_info)
            .then(this.log_service_call('POST', url, {data: reg_info}, null))
            .then(function(response) {
                return response.data;
            });
    },

    /**:Hub.create_change(reg_update_data)
    Updates existing registration

    :param object reg_update_data:
      Gets passed through directly to service. Consult service docs/protocol.
    */
    create_change: function(reg_update_data) {
        var endpoint = 'change/';
        var url = this.base_url + endpoint;

        return this.http_api.post(url, reg_update_data)
            .then(this.log_service_call('POST', url, {data: reg_update_data}, null))
            .then(function(response) {
                return response.data;
            });
    },
